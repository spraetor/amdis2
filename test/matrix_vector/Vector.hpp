...
namespace AMDiS
{
  ...
  public:
    ...
    VectorBase(VectorExpr<Expr> const& expr)
      : Super(size_type(size(expr.sub())))
    {
      MSG("VectorBase->Copy(VectorExpr(" << &expr.sub() << "))");
    ...
    VectorBase(std::initializer_list<value_type> l)
      : Super(l.size())
    {
      TEST_EXIT_DBG( _size == l.size(), "size(Initializer-list) should be size(vector)!");
  ...
  size_t size(VectorBase<M,S> const& vec)
  {
    ...
    std::cout << "size(VectorBase(" << &vec << ")) = " << s << "\n";
    return s;
  }
