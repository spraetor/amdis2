...
namespace AMDiS
{
  ...
  inline size_t size(DOFMatrix const& A)
  {
    return size(A.getBaseMatrix());
  }

  inline size_t num_rows(DOFMatrix const& A)
  {
    return num_rows(A.getBaseMatrix());
  }

  inline size_t num_cols(DOFMatrix const& A)
  {
    return num_cols(A.getBaseMatrix());
  }

  inline void set_to_zero(DOFMatrix& A)
  {
    A.clear();
  }
