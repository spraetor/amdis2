...
namespace AMDiS
{
  ...
  template <class T>
  inline size_t size(const VectorTerm<T>& t)
  {
    return size(t.sub());
  }

  template <class T>
  inline size_t size(const MatrixTerm<T>& t)
  {
    return size(t.sub());
  }

  template <class T>
  inline size_t num_rows(const VectorTerm<T>& t)
  {
    return num_rows(t.sub());
  }

  template <class T>
  inline size_t num_rows(const MatrixTerm<T>& t)
  {
    return num_rows(t.sub());
  }

  template <class M>
  inline size_t num_cols(const VectorTerm<M>& t)
  {
    return num_cols(t.sub());
  }

  template <class M>
  inline size_t num_cols(const MatrixTerm<M>& t)
  {
    return num_cols(t.sub());
  }
