  ...
  namespace io
  {
    namespace VtkVectorWriter
    {
                     ...
                     bool writeAs3dVector)
      {
        ...
        using size_type = Size_t< WorldVector<DOFIterator<double>*> >;
        
        for (size_type i = 0; i < values.getSize(); i++)
          iterators[i] = new DOFIterator<double>(values[i],USED_DOFS);
        for (size_type i = 0; i < iterators.getSize(); i++)
          iterators[i]->reset();
                     ...
                     bool writeAs3dVector)
      {
        ...
        for (size_type i = 0; i < values->getSize(); i++)
          iterators.push_back(new DOFIterator<double>(values->getDOFVector(int( i )),USED_DOFS));
