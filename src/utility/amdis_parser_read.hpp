...
namespace boost { namespace property_tree { namespace amdis_parser
{
    ...
    template<class Ptree>
    struct amdis_grammar :
        public boost::spirit::classic::grammar<amdis_grammar<Ptree> >
    {
        ...
        struct definition
        {
            ...
            boost::spirit::classic::rule<Scanner>
                root, object, member, array, item, value, string, key_string, number;
            boost::spirit::classic::rule<
                typename boost::spirit::classic::lexeme_scanner<Scanner>::type>
                character, escape;

            definition(const amdis_grammar &self)
            {
                ...
                member
                    =   expect_name(key_string[typename Context::a_name(self.c)])
                        >> expect_colon(ch_p(':'))
                        >> expect_value(value)
                        ;
...
//                         >> +(alpha_p | digit_p | chset_p(" _[]"))
                ...
                BOOST_SPIRIT_DEBUG_RULE(key_string);
    ...
    template<class Ptree>
    void read_amdis_internal(std::basic_istream<typename Ptree::key_type::value_type> &stream,
                            Ptree &pt,
                            const std::string &filename)
    {
        ...
        if (!stream.good())
            BOOST_PROPERTY_TREE_THROW(amdis_parser_error("read error", filename, 0));

        // Prepare grammar
        amdis_grammar<Ptree> g;
        ...
        {
            BOOST_PROPERTY_TREE_THROW(amdis_parser_error(e.descriptor, filename, count_lines<It, Ch>(v.begin(), e.where)));
        }
